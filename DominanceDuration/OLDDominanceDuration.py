...
def createRadialStim(winL, winR, resY, Contrast=(LEContrast, REContrast), Condition="Left", units=Units):
    ...
    if simulationMode: # Change stim size in simulation vs lab
        ...
        SizeAdjust = 350
    Contrast = 0.5
    Mask = visual.ShapeStim(winL, vertices=[15,2.5])
    if Condition == "Left": # for displaying on lab
        LeftEye = visual.PatchStim(winL,size=[15, 2.5], ori=45, tex='sin',
                                 sf=0.75, contrast=Contrast, units=units)
        RightEye = visual.PatchStim(winR,size=[2.5, 15], ori=-45,tex='sin',
                                  sf=1, contrast = Contrast, units=units,mask=Mask)

    if Condition == "Right": # for displaying on lab
           RightEye = visual.PatchStim(winR,size=[15, 2.5], ori=45,tex='sin',
                                 sf=0.75,  contrast = Contrast, units=units)
           LeftEye = visual.PatchStim(winL,size=[2.5, 15],ori=-45,tex='sin',
